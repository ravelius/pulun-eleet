...
import {LIVIA_SVG_ELEET,livianSvgAsento,livianSvgKuva} from './livia-svg.js';

// Touhun lisäaika kuuluu itse tekemiselle. Oikeanpuoleinen rata kertoo,
// missä vanhan koreografian kohdassa uusi katseluaika kulkee: esimerkiksi
// nukkuminen, sukiminen tai säältä suojautuminen kestää nyt useita sekunteja.
// Aivastuksen nopeaa pärskähdystä ei venytetä hidastukseksi.
const TOUHUN_AJAT=Object.freeze({
  crumb:{duration:6200,rata:[[0,0],[.17,.18],[.31,.31],[.71,.44],[.85,.72],[1,1]],aktiivi:[.31,.71],sykli:4},
  bread:{duration:6600,rata:[[0,0],[.17,.18],[.31,.31],[.72,.44],[.86,.72],[1,1]],aktiivi:[.31,.72],sykli:4},
  preen:{duration:6000,rata:[[0,0],[.15,.18],[.29,.46],[.77,.62],[.89,.79],[1,1]],aktiivi:[.29,.77],sykli:4},
  sleep:{duration:8300,rata:[[0,0],[.15,.24],[.29,.46],[.82,.82],[.91,.87],[1,1]],aktiivi:[.29,.82],sykli:3},
  wake:{duration:3800,rata:[[0,0],[.28,.08],[.42,.21],[.59,.42],[.82,.56],[1,1]],aktiivi:[.42,.82],sykli:2},
  sneeze:{duration:3900,rata:[[0,0],[.25,.32],[.43,.45],[.53,.47],[.58,.59],[.77,.76],[1,1]],aktiivi:[.36,.77],sykli:2},
  wind:{duration:5000,rata:[[0,0],[.17,.20],[.30,.29],[.82,.69],[.91,.80],[1,1]],aktiivi:[.30,.82],sykli:4},
  rain:{duration:6100,rata:[[0,0],[.16,.20],[.29,.34],[.83,.77],[.92,.85],[1,1]],aktiivi:[.29,.83],sykli:3},
  sun:{duration:5400,rata:[[0,0],[.17,.20],[.34,.51],[.83,.77],[.92,.86],[1,1]],aktiivi:[.34,.83],sykli:3},
  snow:{duration:6100,rata:[[0,0],[.17,.22],[.31,.43],[.82,.77],[.92,.85],[1,1]],aktiivi:[.31,.82],sykli:3},
  bunFeast:{duration:7400,rata:[[0,0],[.12,.16],[.24,.24],[.74,.63],[.84,.72],[.94,.9],[1,1]],aktiivi:[.24,.78],sykli:4},
});
const HAUKOTUKSEN_TAHTI=[[0,0],[.17,.20],[.36,.40],[.58,.52],[.75,.62],[.87,.78],[1,1]];
const KARTAN_NOKKAISUT=[[0,0],[.12,0],[.22,.18],[.285,.22],[.32,1],[.35,1],[.43,0],[.58,0],[.66,.18],[.71,.22],[.745,1],[.775,1],[.85,0],[1,0]];
const KASIN_PIIRRETYT_VERSIOT=[
  ...
  {id:'uusi-yawn',baseId:'yawn',label:'Valtava haukotus',duration:6600,group:'Uudet versiot',kuvaus:'Raskaat luomet, sisäänhengitys ja suuri venytys. Avoin haukotus saa nyt viipyä ennen rauhallista huokausta. Vanha nokka säilyy.'},
...
export const LIVIAN_UUDET_VERSIOT=Object.freeze([
  ...
  ...LIVIA_SVG_ELEET.filter(e=>!kasinTehdyt.has(e.id)).map(e=>Object.freeze({
    id:'uusi-'+e.id,baseId:e.id,label:e.label,duration:e.id==='mapPeck'?4000:TOUHUN_AJAT[e.id]?.duration??e.duration,group:e.group,
    kuvaus:e.label+'. '+(TOUHUN_AJAT[e.id]?'Tekeminen jatkuu pidempään ja saa oman elävän rytminsä. ':'Vanhan Pulun uusi, valmistelun ja jälkiliikkeen sisältävä rytmitys. ')+'Sama liikerata on nyt pelin koodissa.',
  })),
]);

const uudenRajaa=n=>Math.max(0,Math.min(1,Number.isFinite(n)?n:0));
...
export function liikearvo(p,avaimet){
  ...
  for(let i=1;i<avaimet.length;i++){
    ...
    if(p>b)continue;
    const t=uudenRajaa((p-a)/(b-a)),t2=t*t,t3=t2*t;
...
const RADAT={
  'uusi-ilahtuu':{
    ...
    paaKulma:[[0,0],[.12,4],[.155,4],[.225,9],[.31,-8],[.40,-10],[.49,-10],[.57,3],[.68,-7],[.76,-5],[.94,0],[1,0]],
    paaX:[[0,0],[.13,-2],[.23,1],[.36,-3],[.49,-3],[.63,-1],[.95,0],[1,0]],
    paaY:[[0,0],[.135,2],[.22,-6],[.32,-2],[.45,-2],[.58,-4],[.70,2],[.94,0],[1,0]],
...
function vanhanEleenUusiRata(baseId,p){
  ...
  const viive=(nimi,sekuntia)=>liikearvo(uudenRajaa((p-sekuntia)/(1-sekuntia)),liikeperheenRata(ajat,kuva[nimi]||ajat.map(()=>0),voima,nimi==='paa'||nimi==='x'?suunta:1));
...
export function uudenEleenAsento(id,p,{voimakkuus=.5,cueKestoMs=0,playbackRate=1}={}){
  const radat=RADAT[id];
  if(!radat){
    const ele=LIVIAN_UUDET_VERSIOT.find(e=>e.id===id);
    if(!ele)throw new RangeError('Tuntematon katseluele: '+id);
    p=uudenRajaa(p);
    const touhu=TOUHUN_AJAT[ele.baseId],toimintaP=touhu?liikearvo(p,touhu.rata):p;
    const rata=vanhanEleenUusiRata(ele.baseId,toimintaP),voima=.55+.9*uudenRajaa(voimakkuus);
    if(touhu){
      const [alku,loppu]=touhu.aktiivi;
      const paino=liikearvo(p,[[0,0],[alku-.06,0],[alku,1],[loppu,1],[Math.min(.97,loppu+.07),0],[1,0]]);
      const hengitys=Math.sin((p-alku)/(loppu-alku)*Math.PI*2*touhu.sykli)*paino;
      const pieni=ele.baseId==='sleep'?.45:ele.baseId==='sneeze'?.5:1;
      rata.paaKulma+=hengitys*1.4*pieni;
      rata.paaY+=hengitys*1.1*pieni;
      rata.rinta-=hengitys*.8*pieni;
      rata.hengitys+=hengitys*.45*pieni;
    }
    for(const avain of ['paaKulma','paaX','paaY','rinta'])rata[avain]*=voima;
    const perusAsento=livianSvgAsento(ele.baseId,toimintaP,{voimakkuus,cueKestoMs,playbackRate});
    // Vastatuulessa pitkä aktiivivaihe ei ole yksi pitkä silmien sulku:
    // Pulu siristää ja räpäyttää vain puuskien huipuissa.
    if(ele.baseId==='wind'&&p>.34&&p<.82)
      perusAsento.frame=Math.sin((p-.34)*Math.PI*8)>.9?'blink':'glance';
    if((ele.baseId==='bunFeast'||ele.baseId==='bread')&&p>0&&p<1)rata.pulla={p};
    if(ele.baseId==='mapPeck'&&p>0&&p<1){
      const amount=liikearvo(p,KARTAN_NOKKAISUT),peck=p<.5?1:2;
      perusAsento.mapPeck={amount,peck};
      perusAsento.frame=amount>.08?'down':'rest';perusAsento.gazeDown=amount>.08;
      rata.mapContact=Math.max(0,Math.min(1,(amount-.82)/.18));
      rata.mapPeckNumber=peck;
      rata.paaX+=(peck===1?-1.5:1.5)*amount;
    }
    return {id,p,toimintaP,...rata,perusAsento};
  }
  const s={id,p:uudenRajaa(p),paaKulma:0,paaX:0,paaY:0,rinta:0,siipi:0,takasiipi:0,sulat:0,katse:0,ilme:0,rapaytys:0,sivu:0,paperi:0,havahdus:0,kirjaKulma:0,kirjaKiinni:0,kirjaX:0,kirjaY:0,kirjaKallistus:0,vihellys:0,ryhti:0,kyyry:0,lasikorjaus:0,hengitys:0,suusiipi:0,rintasiipi:0};
  const toimintaP=id==='uusi-yawn'?liikearvo(s.p,HAUKOTUKSEN_TAHTI):s.p;
  for(const [avain,rata]of Object.entries(radat))s[avain]=liikearvo(toimintaP,rata);
  const voima=.55+.9*uudenRajaa(voimakkuus);
...
function siipi(s,taka=false){
  const t=taka?s.takasiipi:s.siipi,auki=uudenRajaa(t/.22),kirja=onKirja(s);
  if(!taka&&s.rintasiipi>0){
    const r=uudenRajaa(s.rintasiipi),vanha={...s,rintasiipi:0};
...
function katselukirja(s){
  ...
  const k=uudenRajaa(s.kirjaKiinni);
...
/** Pelin ja katselun yhteinen liikerata, mutta pelin tilakentät säilyvät. */
export function livianUusiPelitila(id,p,asetukset={}){
  const uusi=uudenEleenAsento('uusi-'+id,p,asetukset);
  if(uusi.perusAsento)return {...uusi.perusAsento,katseluRata:uusi};
  return {...livianSvgAsento(id,p,asetukset),uusiPiirto:uusi};
}

/** Asuste ja karttaleijunta säilyttävät oman, ankkuroitavan SVG-piirtonsa. */
export function livianUusiPelikuva(s,asetukset={}){
  if(s.uusiPiirto&&!s.astronautti&&!s.mapHover?.height)
    return uudenEleenKuva(s.uusiPiirto,asetukset);
  return livianSvgKuva(s,asetukset);
}

export const LIVIAN_UUDET_PELIELEET=Object.freeze(LIVIA_SVG_ELEET.map(ele=>Object.freeze({
  ...ele,duration:LIVIAN_UUDET_VERSIOT.find(uusi=>uusi.id==='uusi-'+ele.id)?.duration??ele.duration,
})));
